...
class Solution:
    def maxSubarraySumCircular(self, nums: List[int]) -> int:
        ...
        # invert the nums:
        nums_invert = [-1*j for j in nums]

        # now do kadane's on this inverted nums
        cursum = nums_invert[0]
        ans2 = nums_invert[0]
        for j in range(1,len(nums_invert)):
            cursum = max(nums_invert[j],cursum+nums_invert[j])
            ans2 = max(ans2,cursum)
        
        # so the min for the inverted kadane's will be sum(inv) - max, then invert again to get max
        ans2 = -(sum(nums_invert) - ans2)
...
def main():
    ...
    test = Solution()
    print(test.maxSubarraySumCircular(nums))
